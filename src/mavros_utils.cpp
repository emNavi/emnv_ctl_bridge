...
float racer_x,racer_y,racer_z;
bool swarm_racer_flage;
...
void MavrosUtils::mavPosCtrlSpCallback(const emnv_ctl_bridge::PvayCommand::ConstPtr &msg)
{
    ...
    {
        if(swarm_racer_flage==true)
        {
            ctrl_cmd_.position(0) = msg->position.x - racer_x;
            ctrl_cmd_.position(1) = msg->position.y - racer_y;
            ctrl_cmd_.position(2) = msg->position.z - racer_z;
        }
        else
        {
